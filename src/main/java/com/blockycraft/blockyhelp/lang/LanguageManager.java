package com.blockycraft.blockyhelp.lang;
...
public class LanguageManager {
    ...
    public LanguageManager(BlockyHelp plugin) {
        this.plugin = plugin;
        loadLanguages();
    }
    ...
    public List<String> getCommandList(String lang) {
        List<String> commandList = new ArrayList<>();
        Properties props = messages.getOrDefault(lang, messages.get(defaultLang));
        if (props == null) {
            props = messages.get(defaultLang);
        }

        for (String key : props.stringPropertyNames()) {
            if (key.startsWith("command.") && key.endsWith(".name")) {
                String commandKey = key.substring(0, key.length() - ".name".length());
                String name = props.getProperty(commandKey + ".name", "");
                String description = props.getProperty(commandKey + ".description", "");
                
                if (!name.isEmpty()) {
                    commandList.add(get(lang, "command-format", new HashMap<String, String>() {{
                        put("name", name);
        ...
        commandList.sort(String::compareTo);
        return commandList;
    }
}